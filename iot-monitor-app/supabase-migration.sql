...
ALTER TABLE devices 
  ...
  ADD COLUMN IF NOT EXISTS water_tank_height_cm NUMERIC(10, 2),
  ADD COLUMN IF NOT EXISTS water_tank_radius_cm NUMERIC(10, 2),
  ADD COLUMN IF NOT EXISTS water_sensor_offset_cm NUMERIC(10, 2),
  ADD COLUMN IF NOT EXISTS water_tank_capacity_liters NUMERIC(12, 2);

ALTER TABLE water_readings
  ADD COLUMN IF NOT EXISTS tank_radius_cm NUMERIC(10, 2),
  ADD COLUMN IF NOT EXISTS sensor_offset_cm NUMERIC(10, 2),
  ADD COLUMN IF NOT EXISTS computed_with_device_profile BOOLEAN DEFAULT false;
...
ALTER TABLE water_readings 
  ...
  ADD CONSTRAINT check_water_valid_values 
    CHECK (
      ...
      tank_capacity_liters > 0 AND tank_capacity_liters <= 1000000 AND
      (tank_radius_cm IS NULL OR (tank_radius_cm > 0 AND tank_radius_cm <= 500)) AND
      (sensor_offset_cm IS NULL OR (sensor_offset_cm >= 0 AND sensor_offset_cm <= 200))
    );
